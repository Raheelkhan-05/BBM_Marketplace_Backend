...
export async function getShopBySlug(req, res) {
  ...
  const [{ data: photos }, { data: certifications }] = await Promise.all([
    supabase.from("seller_photos").select("category, url").eq("seller_id", seller.id).order("sort_order"),
    supabase.from("seller_certifications").select("type, name, issued_by, issued_date, file_url").eq("seller_id", seller.id),
  ]);
