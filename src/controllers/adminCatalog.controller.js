...
const LEVEL_CONFIG = {
    ...
    // Canonical identity of a brand item (name + brand + image ONLY).
    // Commercial terms (price/moq/unit/lead time/stock) live entirely on
    // seller_product_submissions, one row per seller, linked via
    // generic_product_brand_id — this level never touches those fields.
    // Shape here is identical to every other simple level (category/
...
export async function getCatalogEntry(req, res) {
    ...
    if (level === "subcategory") {
        ...
        // no name normalization needed anymore — hs_generic_product_brands
        // already has a real "name" column, unlike the old seller_product_submissions detour.
    }
...
// brand_item now goes through the exact same generic path as every other
// level — the old dedicated branch that inserted into
// seller_product_submissions with price/moq/unit/lead_time/is_admin_added
// has been removed entirely. Admin only ever supplies name + brand_name +
// image here; commercial terms are a seller-listing concern, not part of
